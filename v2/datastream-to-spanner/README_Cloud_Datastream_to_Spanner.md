...
* **transformationContextFilePath** : Transformation context file path in cloud storage used to populate data used in transformations performed during migrations   Eg: The mySqlShard id to db name to identify the db from which a row was migrated.
...
* **shardingContextFilePath** : Sharding context file path in cloud storage is used to populate the mySqlShard id in spanner database for each source mySqlShard.It is of the format Map<stream_name, Map<db_name, shard_id>>.
