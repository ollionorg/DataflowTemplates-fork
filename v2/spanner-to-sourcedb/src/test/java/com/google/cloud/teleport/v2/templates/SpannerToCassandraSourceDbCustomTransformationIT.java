...
public class SpannerToCassandraSourceDbCustomTransformationIT extends SpannerToCassandraDbITBase {

  private static final String SPANNER_DDL_RESOURCE =
      "SpannerToCassandraSourceDbCustomTransformationIT/spanner-schema.sql";
  private static final String CASSANDRA_SCHEMA_FILE_RESOURCE =
      "SpannerToCassandraSourceDbCustomTransformationIT/cassandra-schema.sql";
  private static final String CASSANDRA_CONFIG_FILE_RESOURCE =
      "SpannerToCassandraSourceDbCustomTransformationIT/cassandra-config-template.conf";

  private static final String TABLE = "AllDatatypeTransformation";
  ...
  private void writeRowInSpanner() {
    ...
    spannerResourceManager.write(m);
    m = Mutation.delete(TABLE, Key.of("SampleVarchar"));
    spannerResourceManager.write(m);
  }
  ...
  private void assertRowInCassandraDB() throws MultipleFailureException {
    ...
    ResultSet resultSets =
        cassandraResourceManager.executeStatement(String.format("SELECT * FROM %s", TABLE));
    resultSets.forEach(
        resultSet -> {
          try {
            assertAll(
                () -> assertThat(resultSet.getString("varchar_column")).isEqualTo("SampleVarchar"),
                () -> assertThat(resultSet.getByte("tinyint_column")).isEqualTo((byte) 122));
    ...
    PipelineOperator.Result result1 =
        pipelineOperator()
            .waitForCondition(
                createConfig(jobInfo, Duration.ofMinutes(25)), () -> getRowCount() == 0);
