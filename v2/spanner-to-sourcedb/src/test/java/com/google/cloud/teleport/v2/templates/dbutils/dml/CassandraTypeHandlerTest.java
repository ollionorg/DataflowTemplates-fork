...
public class CassandraTypeHandlerTest {
  ...
  public void testHandleUnsupportedMapType() {
    String newValuesString =
        "{\"values\":[{\"key1\":\"value1\"}, {\"key2\":\"value2\"}]}"; // Map (unsupported type)
    JSONObject newValuesJson = new JSONObject(newValuesString);

    try {
      handleFloatSetType(
          "values", newValuesJson); // This should throw the exception for unsupported type
      fail("Expected IllegalArgumentException for unsupported type");
    } catch (IllegalArgumentException e) {
      assertTrue(e.getMessage().contains("Unsupported type for column values"));
    }
  ...
  public void convertSpannerValueJsonToMissingOrNullColumn() {
    ...
    Boolean convertedValue =
        newValuesJson.isNull(colKey) ? false : newValuesJson.optBoolean(colKey, false);
    Assert.assertFalse(convertedValue);
  }
  ...
  public void testHandleStringArrayType() {
    ...
    JSONObject newValuesJson = new JSONObject(newValuesString);
    Set<String> value = handleStringSetType("names", newValuesJson);
  ...
  public void testHandleStringSetType() {
    ...
    JSONObject newValuesJson = new JSONObject(newValuesString);
    List<String> valueList = handleStringArrayType("names", newValuesJson);
  ...
  public void testHandleDateSetType() {
    ...
    JSONObject newValuesJson = new JSONObject(newValuesString);
    // Get the value from the method (this is assumed to return Set<LocalDate>)
    Set<LocalDate> value = handleDateSetType("dates", newValuesJson);

    // Expected value converted to Set<LocalDate>
